/// Demonstrates the Redis pub/sub transport: one bus publishes, another
/// subscribes, both connected to the same Redis instance
...
int main() {
    ...
    try {
        (void)pub_bus.use_transport<conduit::redis::Transport>(pub_cfg);
        (void)sub_bus.use_transport<conduit::redis::Transport>(sub_cfg);
